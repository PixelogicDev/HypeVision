...
def crop(image, controller, current_index):
    ...
    if controller.is_dev:
        ...
        create_dir(f'{current_dir}/python/captures/{current_index}')
        cv2.imwrite(f'{current_dir}/python/captures/{current_index}/720p.png', cropped_720p)
...
def clean(cropped_image, current_index):
    ...
    create_dir(f'{current_dir}/python/captures/{current_index}')
    cv2.imwrite(
        f'{current_dir}/python/captures/{current_index}/cleaned-{current_index}.png', gray_no_noise_threshold)
...
def capture_screen(current_index):
    ...
    if controller.is_dev:
        current_dir = os.getcwd()
        create_dir(f'{current_dir}/python/captures/{current_index}')
        cv2.imwrite(f'{current_dir}/python/captures/{current_index}/screenshot-{current_index}.png', screenshot)

    return (screenshot, current_index + 1)

# This is really just "printing", but will send as string object
def send_prediction(current_index, value):
    data = {
        'num': current_index,
        'prediction': value
    }
...
# Point to bundled version of tesseract
current_dir = os.getcwd()
pytesseract.pytesseract.tesseract_cmd = f'{current_dir}/python/tesseract/tesseract'
...
while (controller.is_capturing):
    ...
    # Send off to client
    send_prediction(current_index, value)
